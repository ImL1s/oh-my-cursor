...
if ((kind !== 'json' && kind !== 'create' && kind !== 'create-text' && kind !== 'text')
  || root === undefined) process.exit(2);

const file = path.join(root, kind === 'text' || kind === 'create-text' ? 'state.txt' : 'state.json');
...
try {
  ...
  if (kind === 'create-text') atomicCreateText(file, 'created\n', { helperFaults: ['after_commit_crash'] });
