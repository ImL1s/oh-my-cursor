...
- 專案狀態：除非傳入 `--init-project-state`，否則不會檢查或建立。

若要初始化或安全採用僅限擁有者的專案狀態根，請加入 `--init-project-state`。既有根必須是目前使用者擁有、權限為 `0700` 的非符號連結目錄；既有 `owner.json`（若有）必須符合 CLI authority schema。
