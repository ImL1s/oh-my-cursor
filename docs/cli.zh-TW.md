...
`--madmax` 對應 Cursor `--force --sandbox disabled --approve-mcps --trust`，並一律以 `--plugin-dir` 載入本套件。這是 host launcher，不是 mode FSM，也不會蓋 `verified`。預設傳輸為 detached tmux 再 attach；auto 在無 tmux 時可回退 direct；顯式 `--tmux` 不會回退。
